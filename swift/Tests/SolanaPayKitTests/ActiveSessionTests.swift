...
struct ActiveSessionTests {
    ...
    func openAndTopupActionFields() async throws {
        ...
        if case let .open(payload) = session.openPullAction(approvedAmount: 5_000_000, owner: "wallet123", approveTxSignature: "approvesig") {
            #expect(payload.mode == .pull)
            #expect(payload.approvedAmount == "5000000")
            #expect(payload.owner == "wallet123")
            #expect(payload.tokenAccount == session.channelIdString())
