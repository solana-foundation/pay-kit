...
/// request once.
///
/// Parallels ``X402Interceptor`` (exact scheme) and `ChargeInterceptor` (MPP
/// charge), so a single `PayKit.HttpClient` shape drives all three. Drive it
/// through `PayKit.HttpClient.x402Upto(signer:)`.
public struct X402UptoInterceptor: PayKit.PaymentInterceptor {
...
public extension PayKit.HttpClient {
    ...
    static func x402Upto(
        ...
        PayKit.HttpClient(
            interceptor: X402UptoInterceptor(signer: signer),
