...
pub async fn build_credential_header_with_options(
    ...
    let request: crate::mpp::protocol::intents::ChargeRequest = challenge
        .request
        .decode()
...
fn challenge_is_unknown_token_2022(
    ...
    let mint = match crate::mpp::protocol::solana::resolve_stablecoin_mint(
        &request.currency,
        method_details.network.as_deref(),
    ) {
        Some(m) => m,
...
fn build_spl_instructions(
    ...
    if token_program.to_string() == programs::TOKEN_2022_PROGRAM
        && !crate::mpp::protocol::solana::is_known_stablecoin_mint(spl)
    ...
    match network {
        ...
        Some(expected) => {
            method_details
                .network
                .as_deref()
                .unwrap_or(crate::mpp::protocol::solana::DEFAULT_NETWORK)
                == expected
        }
    }
}
...
fn currencies_match(
    ...
    crate::mpp::protocol::solana::resolve_stablecoin_mint(challenge_currency, network)
        == crate::mpp::protocol::solana::resolve_stablecoin_mint(expected_currency, network)
}
...
mod tests {
    ...
    fn parse_challenge_from_header() {
        ...
        // Decode the request
        let req: crate::mpp::protocol::intents::ChargeRequest = parsed.request.decode().unwrap();
    ...
    fn resolve_mint_known_symbols() {
        ...
        assert_eq!(
            ...
            Some(crate::mpp::protocol::solana::mints::USDT_MAINNET)
        );
        assert_eq!(
            resolve_mint("CASH", None),
            Some(crate::mpp::protocol::solana::mints::CASH_MAINNET)
        );
        assert_eq!(
            resolve_mint("USDPT", None),
            Some(crate::mpp::protocol::solana::mints::USDPT_MAINNET)
        );
    }
    ...
    async fn build_charge_confidential_resolves_currency_symbol() {
        ...
        let md = MethodDetails {
            ...
            token_program: Some(
                crate::mpp::protocol::solana::programs::TOKEN_2022_PROGRAM.to_string(),
            ),
    ...
    async fn build_charge_too_many_splits() {
        ...
        assert!(matches!(err, Err(crate::mpp::Error::TooManySplits)));
    }
    ...
    async fn build_charge_splits_exceed_amount() {
        ...
        let err =
            build_charge_transaction(signer.as_ref(), &rpc, "1000000", "SOL", RECIPIENT, &md).await;
        assert!(matches!(err, Err(crate::mpp::Error::SplitsExceedAmount)));
    }
    ...
    async fn build_charge_transaction_fails_closed_on_confidential() {
        ...
        let err = build_charge_transaction_with_options(
            ...
            &rpc,
            "1000000",
            crate::mpp::protocol::solana::mints::USDPT_MAINNET,
            RECIPIENT,
            &md,
            BuildChargeTransactionOptions::default(),
        )
