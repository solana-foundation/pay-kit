...
pub use crate::core::MAX_DECIMALS;
...
pub fn parse_units(amount: &str, decimals: u8) -> Result<String, crate::mpp::error::Error> {
...
pub fn deserialize_request<T: serde::de::DeserializeOwned>(
    request_b64: &str,
) -> Result<T, crate::mpp::error::Error> {
    let bytes = crate::mpp::protocol::core::base64url_decode(request_b64)?;
    serde_json::from_slice(&bytes)
        .map_err(|e| crate::mpp::error::Error::Other(format!("Failed to deserialize request: {e}")))
}

/// Serialize a request to a base64url JSON string.
pub fn serialize_request<T: serde::Serialize>(
    request: &T,
) -> Result<String, crate::mpp::error::Error> {
    let json = serde_json_canonicalizer::to_string(request)
        .map_err(|e| crate::mpp::error::Error::Other(format!("Canonical JSON failed: {e}")))?;
    Ok(crate::mpp::protocol::core::base64url_encode(
        json.as_bytes(),
    ))
}
...
mod tests {
    ...
    fn deserialize_request_invalid_json() {
        let encoded = crate::mpp::protocol::core::base64url_encode(b"not json");
    ...
    fn deserialize_request_wrong_type() {
        let encoded = crate::mpp::protocol::core::base64url_encode(b"{\"x\": 1}");
