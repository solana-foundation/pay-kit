...
impl PaymentChallenge {
    ...
    pub fn to_header(&self) -> Result<String, crate::mpp::error::Error> {
        super::format_www_authenticate(self)
    }

    /// Parse from a WWW-Authenticate header value.
    pub fn from_header(header: &str) -> Result<Self, crate::mpp::error::Error> {
...
impl PaymentCredential {
    ...
    pub fn payload_as<T: serde::de::DeserializeOwned>(
        ...
        serde_json::from_value(self.payload.clone()).map_err(|e| {
            crate::mpp::error::Error::Other(format!("payload deserialization failed: {e}"))
        })
    }
