...
fn v1_network_for_requirements(requirements: &PaymentRequirements) -> &'static str {
    ...
    {
        "devnet" | "solana-devnet" | crate::x402::protocol::schemes::exact::SOLANA_DEVNET => {
            "solana-devnet"
        }
        _ => SOLANA_NETWORK,
    }
...
mod tests {
    ...
    fn test_requirements(currency: &str) -> PaymentRequirements {
        PaymentRequirements {
            ...
            decimals: if crate::x402::protocol::schemes::exact::is_native_sol(currency) {
                None
            } else {
                Some(6)
            },
    ...
    fn parse_x402_express_body() {
        ...
        assert_eq!(
            ...
            Some(crate::x402::protocol::schemes::exact::SOLANA_DEVNET)
        );
    ...
    fn parse_x402_challenge_from_body() {
        ...
        assert_eq!(
            req.cluster.as_deref(),
            Some(crate::x402::protocol::schemes::exact::SOLANA_MAINNET)
        );
    }
