...
| Concern | Location |
| --- | --- |
| Crypto primitives, key derivation, `recover_split_amount`, litesvm e2e | `pay-kit/rust/crates/kit/src/mpp/protocol/confidential.rs` |
| Protocol types: `MethodDetails`, `CredentialPayload::Bundle`, `validate_confidential_charge` | `pay-kit/rust/crates/kit/src/mpp/protocol/solana.rs` |
| Client bundle builder (`build_confidential_transfer_bundle`, spl-record staging) | `pay-kit/rust/crates/kit/src/mpp/client/confidential.rs` |
| Server settlement (`settle_confidential_bundle`, allow-list `verify_confidential_bundle_tx`, two modes) | `pay-kit/rust/crates/kit/src/mpp/server/charge.rs` |
| Confidential worker run-loop + orphan sweeper (`worker` feature) | `pay-kit/rust/crates/kit/src/mpp/server/confidential_worker.rs` + `sweep_confidential_orphans` in `charge.rs` |
| Surfpool e2e integration tests (full lifecycle, both settlement modes, sweep) | `pay-kit/rust/crates/integration-tests/tests/confidential_integration.rs` |
