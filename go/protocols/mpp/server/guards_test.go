...
func TestValidateSplitsCount(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			splits := make([]paycore.Split, tc.count)
			...
			if tc.wantErr {
				...
				if sdkErr.Code != core.ErrCodeTooManySplits {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				if sdkErr.Code != core.ErrCodeComputeBudgetExceeded {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				if sdkErr.Code != core.ErrCodeInvalidPayload {
					t.Fatalf("code = %q, want %q", sdkErr.Code, core.ErrCodeInvalidPayload)
				}
