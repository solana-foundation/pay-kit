...
func VerifyOpenTx(ctx context.Context, expected VerifyOpenTxExpected, payload *intents.OpenPayload, rpcClient solanatx.RPCClient) (VerifyOpenTxResult, error) {
	...
	var openArgs pcgen.OpenArgs
	if err := bin.NewBorshDecoder(openIx.Data[1:]).Decode(&openArgs); err != nil {
...
func openSplits(entries []pcgen.DistributionEntry) []paymentchannels.Distribution {
...
// NewTopUpTxVerifier is the legacy payload-only callback factory. Keep this
// public shape for host integrations that install VerifyTopUpTx directly.
// Session construction uses NewTopUpStateTxVerifier below for account binding.
