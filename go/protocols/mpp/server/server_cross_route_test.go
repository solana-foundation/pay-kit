...
func resignEcho(secret string, echo *core.ChallengeEcho) {
	echo.ID = core.ComputeChallengeID(
		secret,
		echo.Realm,
		string(echo.Method),
		string(echo.Intent),
...
func signatureCredentialFromEcho(t *testing.T, echo core.ChallengeEcho) core.PaymentCredential {
	t.Helper()
	cred, err := core.NewPaymentCredential(echo, map[string]any{
		"type":      "signature",
...
func opaqueRaw(o *core.Base64URLJSON) string {
	if o == nil {
		return ""
	}
	return o.Raw()
...
func TestVerifyCredentialTier2RejectsTamperedMethod(t *testing.T) {
	...
	echo := challenge.ToEcho()
	echo.Method = core.NewMethodName("stripe")
	resignEcho(cfg.SecretKey, &echo)
...
func TestVerifyCredentialTier2RejectsNonChargeIntent(t *testing.T) {
	...
	echo := challenge.ToEcho()
	echo.Intent = core.NewIntentName("session")
	resignEcho(cfg.SecretKey, &echo)
...
func TestVerifyCredentialTier2RejectsTamperedCurrency(t *testing.T) {
	...
	tamperedReq, err := core.NewBase64URLJSONValue(req)
	if err != nil {
...
func TestVerifyCredentialWithExpectedRejectsAmountMismatch(t *testing.T) {
	...
	var paymentErr *core.Error
	if !mppErrAs(err, &paymentErr) {
		t.Fatalf("expected core.Error, got %T: %v", err, err)
	}
	if paymentErr.Code != core.ErrCodeAmountMismatch {
...
func mppErrAs(err error, target **core.Error) bool {
	if err == nil {
		return false
	}
	if pe, ok := err.(*core.Error); ok {
		*target = pe
		return true
	}
...
func TestRequestRoundTrip(t *testing.T) {
	...
	v, err := core.NewBase64URLJSONValue(in)
	if err != nil {
...
func TestVerifyCredentialWithExpectedRejectsCurrencyMismatchAssertsCode(t *testing.T) {
	...
	var paymentErr *core.Error
	if !mppErrAs(err, &paymentErr) || paymentErr.Code != core.ErrCodeChallengeRouteMismatch {
...
func TestVerifyCredentialWithExpectedRejectsRecipientMismatchAssertsCode(t *testing.T) {
	...
	var paymentErr *core.Error
	if !mppErrAs(err, &paymentErr) || paymentErr.Code != core.ErrCodeRecipientMismatch {
