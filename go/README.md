...
[`tests/harness/go-server/main.go`](../tests/harness/go-server/main.go)
...
The cross-language interop harness lives in `../tests/harness`. The Go
SDK ships both a client (`tests/harness/go-client`) and a server
(`tests/harness/go-server`) adapter. Both are opt-in via the
