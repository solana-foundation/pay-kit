...
def require_usage(
    ...
    *,
    ...
    :class:`~pay_kit.usage.Charge` meter to ``request.charge`` (read it with
    :func:`charge` or :func:`pay_kit.usage.charge_from`); after the view it
...
def charge(request: HttpRequest) -> Charge | None:
    """Return the usage :class:`~pay_kit.usage.Charge` meter on ``request``, or ``None``."""
    from pay_kit.usage import charge_from
