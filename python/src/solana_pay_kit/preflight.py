...
Two checks run at ``configure`` time, mirroring Ruby ``solana_pay_kit/preflight.rb``
...
missing account raises :class:`~solana_pay_kit.errors.ConfigurationError` at boot so
...
``solana_pay_kit.preflight.run`` and inject a fake RPC callable via
...
_LOG = logging.getLogger("solana_pay_kit.preflight")
...
def run(config: Config) -> None:  # pragma: no cover - live RPC + Surfnet cheatcodes
    ...
    try:
        ...
        _LOG.warning("[solana_pay_kit preflight] skipped fee-payer balance check: %s", exc)
    ...
    for coin in config.stablecoins:
        try:
            ...
            _LOG.warning("[solana_pay_kit preflight] skipped %s ATA check: %s", coin, exc)
...
def _check_fee_payer_sol(config: Config, autofix: bool) -> None:  # pragma: no cover - live RPC
    ...
    if autofix:
        _LOG.info(
            "[solana_pay_kit preflight] funding demo fee-payer %s with %d lamports via surfnet_setAccount",
            pubkey,
            AUTOFUND_LAMPORTS,
        )
        _rpc_call(
    ...
    raise ConfigurationError(
        f"solana_pay_kit preflight: fee-payer {pubkey} has {lamports} lamports on "
...
def _check_recipient_ata(config: Config, coin: str, autofix: bool) -> None:  # pragma: no cover - live RPC
    ...
    if autofix:
        _LOG.info(
            "[solana_pay_kit preflight] provisioning %s ATA for %s (mint=%s) via surfnet_setTokenAccount",
            coin,
            recipient,
            mint,
        )
    ...
    raise ConfigurationError(
        f"solana_pay_kit preflight: recipient {recipient} has no {coin} ATA on "
