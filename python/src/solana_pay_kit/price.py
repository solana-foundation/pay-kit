...
from solana_pay_kit.errors import ConfigurationError, MixedCurrenciesError
...
def _to_decimal(amount: object) -> Decimal:
    ...
    if isinstance(amount, bool):  # bool is an int subclass; reject explicitly.
        raise ConfigurationError("solana_pay_kit: Price amount must be str | int | Decimal, not bool")
    if isinstance(amount, float):
        raise ConfigurationError("solana_pay_kit: Price amount must be str | int | Decimal, not float")
    if isinstance(amount, Decimal):
        ...
        if not _AMOUNT_RE.match(amount):
            raise ConfigurationError(f"solana_pay_kit: invalid Price amount: {amount!r}")
        try:
            coerced = Decimal(amount)
        except InvalidOperation as exc:
            raise ConfigurationError(f"solana_pay_kit: invalid Price amount: {amount!r}") from exc
    else:
        raise ConfigurationError("solana_pay_kit: Price amount must be str | int | Decimal")
    ...
    # allowed (a free gate).
    if coerced < 0:
        raise ConfigurationError(f"solana_pay_kit: Price amount must not be negative: {amount!r}")
    return coerced
