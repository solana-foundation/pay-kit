...
base :class:`~solana_pay_kit.price.Price`, an optional ``pay_to`` override (falling
...
6. x402 is auto-disabled when fees are present (stock x402 facilitators
   ...
   fee-bearing gate raises :class:`~solana_pay_kit.errors.ProtocolIncompatibleError`,
...
def _build_fees(
    ...
    for kind, mapping in pairs:
        ...
        if not isinstance(mapping, dict):
            raise ConfigurationError(f"solana_pay_kit: fee_{kind} must be a dict of {{recipient: Price}}")
...
def _resolve_accept(
    ...
    if Protocol.X402 in accept:
        raise ProtocolIncompatibleError(
            f"solana_pay_kit: gate {name!r}: x402 cannot be combined with fees "
            f"(stock x402 facilitators settle to a single address). "
            f"Drop Protocol.X402 from accept or remove the fees."
        )
    if not accept:
        raise ProtocolIncompatibleError(
            f"solana_pay_kit: gate {name!r}: fees present and x402 auto-disabled, "
...
class Gate(pydantic.BaseModel):
    ...
    def build(
        ...
        *,
        ...
        Raises :class:`~solana_pay_kit.errors.ConfigurationError` (and subclasses) on
        ...
        if not isinstance(name, str) or not name:  # pyright: ignore[reportUnnecessaryIsInstance]
            raise ConfigurationError(f"solana_pay_kit: gate name must be a non-empty string, got {name!r}")
        if not isinstance(amount, Price):  # pyright: ignore[reportUnnecessaryIsInstance]
            raise ConfigurationError(f"solana_pay_kit: gate {name!r}: amount must be a Price (use usd/eur/gbp)")
        ...
        if accept is None and accept_default is not None and not accept_default:
            raise ConfigurationError(f"solana_pay_kit: gate {name!r}: accept resolved to an empty list")
    ...
    def _validate_fee_recipients(name: str, pay_to: str, fees: tuple[Fee, ...]) -> None:
        ...
        for fee in fees:
            if fee.recipient == pay_to:
                raise ConfigurationError(
                    f"solana_pay_kit: gate {name!r}: fee recipient {pay_to!r} duplicates "
                    f"pay_to; fold the fee into the amount instead"
                )
            if fee.recipient in seen:
                raise ConfigurationError(f"solana_pay_kit: gate {name!r}: duplicate fee recipient {fee.recipient!r}")
        ...
        for fee in fees:
            if fee.price.currency != amount.currency:
                raise MixedCurrenciesError(
                    f"solana_pay_kit: gate {name!r}: fee for {fee.recipient!r} is "
    ...
    def _validate_within_sum(name: str, amount: Price, fees: tuple[Fee, ...]) -> None:
        ...
        if within_sum > amount.amount:
            raise ConfigurationError(
                f"solana_pay_kit: gate {name!r}: sum(fee_within) = {within_sum} exceeds amount {amount.amount_string()}"
            )
