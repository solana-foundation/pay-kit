...
the one the solana_pay_kit x402 server reads (``Payment-Signature``; confirmed in
``solana_pay_kit.protocols.x402._payment_signature_header``).
"""
...
#: Request header the solana_pay_kit x402 server reads the credential from.
...
class PaymentTransport(httpx.AsyncBaseTransport):
    ...
    async def handle_async_request(self, request: httpx.Request) -> httpx.Response:
        ...
        try:
            ...
            logger.warning("solana_pay_kit: failed to build x402 payment credential", exc_info=True)
            return response
