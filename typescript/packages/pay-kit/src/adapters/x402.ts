...
export function createX402ExactAdapter(config: PayKitConfig): ProtocolAdapter {
    ...
    return {
        ...
        challengeHeaders(gate: Gate, request: Request): Promise<Readonly<Record<string, string>>> {
            const paymentRequired: PaymentRequired = {
                accepts: [requirementsFor(gate)],
                resource: { url: new URL(request.url).pathname },
                x402Version: X402_VERSION,
            };
            return Promise.resolve({ [PAYMENT_REQUIRED_HEADER]: encodePaymentRequiredHeader(paymentRequired) });
        },
