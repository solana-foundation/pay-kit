...
class TransactionTest < Minitest::Test
  ...
  def test_parses_and_serializes_legacy_transaction
    ...
    tx = Mpp::Solana::Transaction.from_bytes(raw)
  ...
  def test_parses_v0_transaction_without_address_lookups
    ...
    [payer, recipient, PROGRAMS::SYSTEM_PROGRAM].each { |key| message << Mpp::Solana::Base58.decode(key) }
    message << Mpp::Solana::Base58.decode(pubkey(9))
  ...
  def test_rejects_truncated_transaction
    assert_raises(ArgumentError) { Mpp::Solana::Transaction.from_bytes("\x01\x00".b) }
  end

  def test_rejects_unsupported_version_and_signer_not_found
    raw = compact_u16(0) + [0x81, 1, 0, 0].pack("C*")
    assert_raises(ArgumentError) { Mpp::Solana::Transaction.from_bytes(raw) }

    tx = Mpp::Solana::Transaction.from_bytes(legacy_transaction(
      account_keys: [pubkey(1), pubkey(2), PROGRAMS::SYSTEM_PROGRAM],
      instructions: [compiled_instruction(2, [0, 1], u32(2) + u64(1000))]
    ))
    keypair = Mpp::Solana::Keypair.new(Array.new(64, 9))
  ...
  def test_from_base64_invalid_and_cursor_boundaries
    assert_raises(ArgumentError) { Mpp::Solana::Transaction.from_base64("%%%") }
    assert_equal [0x80, 0x01].pack("C*"), Mpp::Solana::Transaction.compact_u16(128)
    cursor = Mpp::Solana::Cursor.new("\xff\xff\xff\xff".b)
    assert_raises(ArgumentError) { cursor.compact_u16 }
    assert_raises(ArgumentError) { Mpp::Solana::Cursor.new("").peek }
    assert_raises(ArgumentError) { Mpp::Solana::Cursor.new("").byte }
    assert_raises(ArgumentError) { Mpp::Solana::Cursor.new("a").bytes(2) }
  end
  ...
  def test_derives_associated_token_address
    ...
    ata = Mpp::Solana::AssociatedToken.derive(
      owner: owner,
      mint: mint,
  ...
  def test_program_address_derivation_handles_high_bump_bytes
    ...
    _address, bump = Mpp::Solana::PublicKey.find_program_address(["seed"], program_id)
