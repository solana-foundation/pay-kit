...
module Mpp
  module Server
    # User-facing MPP charge server. Build one with `Mpp.create(method:, ...)`.
    #
    ...
    class Charge
      ...
      # Not part of the public API. Drive this through `Mpp.create` + `Charge#charge`.
      class Handler
        ...
        def handle(authorization, request)
          ...
          ::Mpp::Settlement.new(
            ...
            headers: {
              ::Mpp::Protocol::Core::Headers::PAYMENT_RECEIPT => receipt,
              settlement_header => signature
            }
          )
        rescue ArgumentError, ::Mpp::Error, ::PayCore::Solana::Rpc::RpcError, ::PayCore::Solana::Transaction::SigningError => error
        ...
        def settle_payload(credential, request)
          ...
          raise ::Mpp::VerificationError, "missing transaction or signature payload" unless signature.is_a?(String) && !signature.empty?
          ...
          raise ::Mpp::VerificationError, verification.reason unless verification.ok?
        ...
        def settle_pull(transaction_base64)
          ...
          raise ::Mpp::VerificationError, "Simulation failed: #{simulation["err"].inspect}" unless simulation["err"].nil?
        ...
        def fetch_settled_transaction(signature)
          @confirmation_attempts.times do
            ...
            meta = response["meta"]
            raise ::Mpp::VerificationError, "getTransaction response is missing transaction metadata" unless meta.is_a?(Hash)
            raise ::Mpp::VerificationError, "Transaction #{signature} failed: #{meta["err"].inspect}" unless meta["err"].nil?

            wire = response["transaction"]
            return wire[0] if wire.is_a?(Array) && wire[0].is_a?(String) && !wire[0].empty?

            raise ::Mpp::VerificationError, "getTransaction response is missing base64 transaction"
          end
          raise ::Mpp::VerificationError, "Timed out fetching transaction #{signature}"
        end
        ...
        def await_confirmation(signature)
          @confirmation_attempts.times do
            ...
            if status.is_a?(Hash)
              raise ::Mpp::VerificationError, "Transaction #{signature} failed: #{status["err"].inspect}" unless status["err"].nil?
              return if ["confirmed", "finalized"].include?(status["confirmationStatus"])
            end
            sleep @confirmation_delay
          end
          raise ::Mpp::VerificationError, "Timed out waiting for transaction #{signature}"
        end
        ...
        def consume_signature(signature)
          ...
          raise ::Mpp::VerificationError.new("Transaction signature already consumed", code: ::PayCore::ErrorCodes::CODE_SIGNATURE_CONSUMED) unless inserted
        end
        ...
        def check_network_blockhash(blockhash)
          ...
          raise ::Mpp::VerificationError.new("Signed against localnet but the server expects #{network}. Switch your client RPC to #{network} and re-sign.", code: ::PayCore::ErrorCodes::CODE_WRONG_NETWORK)
        end
      end
    end
  end
