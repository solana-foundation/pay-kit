...
the MPP adapter IDs also implement x402. `batch-settlement` has its own intent
(`MPP_HARNESS_INTENTS=x402-batch-settlement`, scenarios in
`harness/src/intents/x402-batch-settlement.ts`); a scheme without an intent of
its own needs one added first rather than being substituted into these commands.
