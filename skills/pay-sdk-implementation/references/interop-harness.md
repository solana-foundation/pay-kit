...
at `mpp-sdk/tests/harness`. Read its README first
(`tests/harness/README.md`) — that is the contract; this file summarizes
...
1. **Client adapter** — accepts a `MPP_INTEROP_TARGET_URL`, pays it,
...
- `tests/harness/rust-client/` — Cargo manifest wrapper used by the
  harness command.

## The contract (verbatim from `tests/harness/README.md`)
...
- `type`: `"ready"`
- `implementation`: stable id (matches `tests/harness/src/implementations.ts`)
- `role`: `"server"`
...
| Var | Used by | Meaning |
|---|---|---|
| `MPP_INTEROP_RPC_URL` | both | Surfpool RPC URL (localhost:8899) |
| `MPP_INTEROP_NETWORK` | both | `localnet` |
| `MPP_INTEROP_MINT` | both | SPL mint used by the scenario |
| `MPP_INTEROP_PRICE` | both | display price (decimal string, e.g. `"0.001"`) |
| `MPP_INTEROP_SECRET_KEY` | server | deterministic HMAC secret |
| `MPP_INTEROP_CLIENT_SECRET_KEY` | client | JSON array of bytes (Solana keypair) |
| `MPP_INTEROP_FEE_PAYER_SECRET_KEY` | server | JSON array of bytes |
| `MPP_INTEROP_PAY_TO` | server | expected recipient pubkey |
| `MPP_INTEROP_TARGET_URL` | client | URL to pay |
| `MPP_INTEROP_REPLAY_SOURCE_PATH` | server | optional cheaper source route (cross-route replay test) |
| `MPP_INTEROP_REPLAY_SOURCE_PRICE` | server | optional source route price |
| `MPP_INTEROP_REPLAY_SOURCE_AMOUNT` | server | optional source route amount |
...
Add an entry to `tests/harness/src/implementations.ts` — one each for
client and server:
...
export const clientImplementations: ImplementationDefinition[] = [
  // ...
  {
    ...
    enabled: isEnabled("<lang>", "MPP_INTEROP_CLIENTS", false),
  },
];
...
export const serverImplementations: ImplementationDefinition[] = [
  // ...
  {
    ...
    enabled: isEnabled("<lang>", "MPP_INTEROP_SERVERS", false),
  },
];
```
...
Then drop an adapter wrapper in `tests/harness/<lang>-client/` with
whatever scaffold the language needs (e.g. a `Cargo.toml` that
path-depends on `../../<lang>`, or a `package.json` with a single
`start` script). The harness command is relative to `tests/harness`.
...
MPP_INTEROP_CLIENTS=<lang> MPP_INTEROP_SERVERS=rust pnpm test

# Rust client against the new server
MPP_INTEROP_CLIENTS=rust MPP_INTEROP_SERVERS=<lang> pnpm test

# self-pair
MPP_INTEROP_CLIENTS=<lang> MPP_INTEROP_SERVERS=<lang> pnpm test
```

Add corresponding `Run <lang> client interop smoke` /
`Run <lang> server interop smoke` / `Run <lang> end-to-end interop smoke`
steps to the `interop` job in `.github/workflows/ci.yml`. The Rust steps
...
- **`responseHeaders` is a flat string-keyed object**, not an array of
  ...
  `rust/src/bin/interop_client.rs:22-32`) to handle them — do not
  rely on a single header.
- **`settlement` is optional** but the harness expects it whenever
  the server returns the `interopScenario.settlementHeader` header.
...
- **The replay-source env vars are optional**. The harness uses them to
  ...
  `rust/src/bin/interop_server.rs` for the wiring.
