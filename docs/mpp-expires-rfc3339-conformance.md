...
`Session.ts:356` `assertChallengeOpenNotExpired` evaluates `Date.parse(expires)` at `:358`. Both resolve
at `48ea8aa`, are the same V8 algorithm, and return byte-identical verdicts on all 124 scenarios.
...
# typescript  from typescript/;  drives `parseRfc3339`, so needs #286 in the worktree
npx vitest run packages/mpp/src/__tests__/client-charge-validation.test.ts # exit 1, 6 FAIL of 148
