...
`tests/harness/test/compute-budget-caps.test.ts` parses each file above
...
3. Append a row to `SDKS` in
   `tests/harness/test/compute-budget-caps.test.ts` and to the table
   above. Append a fixture row to
   `charge-compute-budget-over-cap` in
   `tests/harness/src/intents/charge.ts` once the SDK is wired into the
   interop harness.
