...
use PayKit\Pricing;
use PayKit\Scheme;
use PayKit\Schemes\Mpp\Adapter as MppAdapter;
use PayKit\Schemes\X402\Adapter as X402Adapter;
...
final class RequirePayment implements MiddlewareInterface
{
    ...
    private function pickAdapter(Gate $gate, ServerRequestInterface $request): ?object
    {
        ...
        foreach ($accept as $scheme) {
            if ($scheme === Scheme::X402 && $sig !== '' && $this->x402 !== null) {
                return $this->x402;
            }
            if ($scheme === Scheme::Mpp && $auth !== '' && stripos($auth, 'payment ') === 0) {
                return $this->mpp;
            }
        }
        return null;
    ...
    private function build402(Gate $gate, ServerRequestInterface $request): ResponseInterface
    {
        ...
        $stream = Psr17::streamFactory()->createStream(json_encode($body, JSON_THROW_ON_ERROR));
