...
import com.solana.mpp._paycore.AccountMeta
import com.solana.mpp._paycore.Instruction
 ...
 * the ``_paycore`` instruction/transaction codec used by this client.
 *
 ...
 *
 * What web3-solana does NOT supply, and so stays hand-rolled in ``_paycore``:
 *
 ...
 *    so ``_paycore.Transaction.buildV0Message`` keeps doing the compaction.
 *  - **ComputeBudget.** Absent from the web3-solana jar entirely; the two
 *    ComputeBudget instructions stay in ``_paycore.Instructions``.
 *  - **Associated Token Account derivation.** web3-solana only exposes a
 *    ``suspend`` ``ProgramDerivedAddress.find``; the x402 build path is
 *    synchronous, so ``_paycore.Pda.associatedTokenAddress`` derives the ATAs.
 ...
 *    diverges from the spine; those stay on the ``_paycore`` builders that
 *    match the rust bytes.
 */
internal object Web3Solana {
    /**
     * Builds an SPL ``transferChecked`` instruction via web3-solana's
     * [TokenProgram] and lowers it into a [_paycore][Instruction] for the v0
     * codec.
     *
    ...
    fun transferChecked(
        ...
        // _paycore instruction with the offer's program id so Token-2022
        // offers are honoured.
        return ix.toPaycore(programIdOverride = tokenProgram)
    }

    /** Lowers a web3-solana [TransactionInstruction] into a [_paycore][Instruction]. */
