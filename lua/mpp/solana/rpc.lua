...
broadcast/await flow needed by `mpp.server.charge_handler`:
...
A reference `socket.http` transport is provided in `mpp.solana.rpc_transport`
(PR B) for example servers; this module deliberately does not depend on it
to keep `mpp.solana.rpc` itself test-only and pure-Lua.
