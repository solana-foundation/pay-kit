...
helper.test('resty.pay_kit.util.base58 re-exports the legacy mpp.util.base58', function()
  local b58 = require('resty.pay_kit.util.base58')
  helper.assert_equal(type(b58.encode), 'function')
  helper.assert_equal(type(b58.decode), 'function')
end)

helper.test('resty.pay_kit.util.base64url re-exports the legacy module', function()
  local b64u = require('resty.pay_kit.util.base64url')
  helper.assert_equal(type(b64u.encode), 'function')
  helper.assert_equal(type(b64u.decode), 'function')
end)

helper.test('resty.pay_kit.util.json re-exports the canonical JSON module', function()
  local j = require('resty.pay_kit.util.json')
  helper.assert_true(type(j.encode) == 'function' or type(j.decode) == 'function'
    or type(j) == 'table')
end)

helper.test('resty.pay_kit.util.crypto exposes ed25519 + hmac surface', function()
  local crypto = require('resty.pay_kit.util.crypto')
...
helper.test('resty.pay_kit.solana.rpc re-exports the cosocket RPC client', function()
  local rpc = require('resty.pay_kit.solana.rpc')
