...
def interop_config
  ...
  # `X402::Server::Exact::Config.new(rpc_url: ..., pay_to: ..., ...)`
  # with typed kwargs directly.
  @interop_config ||= X402::Server::Exact::Config.from_interop_env
end
...
puts JSON.generate(
  X402::Server::Exact::CAPABILITY_PAYLOAD.merge(type: "ready", port: server.addr[1])
)
$stdout.flush
...
while running
  begin
    ...
    begin
      ...
      status, response_headers, body = X402::Server::Exact.response_for(path, headers, interop_config)
